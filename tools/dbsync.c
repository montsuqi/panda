...
static ARG_TABLE option[] = {
    ...
    {"master", STRING, TRUE, (void *)&Master, "master dbg name"},
    {"slave", STRING, TRUE, (void *)&Slave, "slave dbg name"},

    {"allsync", BOOLEAN, TRUE, (void *)&fAllsync, "All Database sync"},
    ...
    {"v", BOOLEAN, TRUE, (void *)&fOVerbose, "Verbose mode"},
    {NULL, 0, FALSE, NULL, NULL}};

static Bool FirstNG = TRUE;

static void verPrintf(char *format, ...) {
  va_list va;
  if (fVerbose) {
    va_start(va, format);
    vprintf(format, va);
...
static void ms_print(char *name, char *master, char *slave) {
  verPrintf("| %-6s   | %-20s       |     | %-20s       |\n", name,
            (master ? master : ""), (slave ? slave : ""));
}

static void msr_print(char *result, char *master, char *m_count, char *slave,
                      char *s_count) {
  verPrintf("| %-6s   | %-17.17s %8s | ==> | %-17.17s %8s |\n", result, master,
            m_count, slave, s_count);
}

static void info_print(DBG_Struct *master_dbg, DBG_Struct *slave_dbg) {
  separator();
...
static void pre_print(DBG_Struct *master_dbg, DBG_Struct *slave_dbg) {
  if (FirstNG) {
...
static Bool dbtype_check(DBG_Struct *dbg) {
  Bool rc = FALSE;
...
static Bool all_allsync(DBG_Struct *master_dbg, DBG_Struct *slave_dbg) {
  Bool ret;
  DBInfo *dbinfo;
  ...
  if ((!template1_check(slave_dbg)) || (dbactivity(slave_dbg) > 0)) {
    ...
    verPrintf("Create database name=%s, template=%s, encoding=%s, "
              "lc_collate=%s, lc_ctype=%s \n",
              slave_dbg->name, template, encoding, lc_collate, lc_ctype);
    ret = createdb(slave_dbg, tablespace, template, encoding, lc_collate,
                   lc_ctype);
    if (!ret) {
...
static void add_ng_list(TableList *ng_list, Table *ng_table, char ngkind) {
  Table *table;
...
static char *table_name(TableList *table_list, int num) {
  char *ret;
...
static char table_relkind(TableList *table_list, int num) {
  char ret;
  if (table_list->count > num) {
...
static TableList *table_check(DBG_Struct *master_dbg, DBG_Struct *slave_dbg) {
  ...
  for (i = 0; (master_list->count > m) || (slave_list->count > s); i++) {
    ...
    rcmp = (table_relkind(master_list, m) - table_relkind(slave_list, s)) * 10 +
           cmp;
    if (rcmp == 0) {
      if (!strcmp(master_list->tables[m]->count,
                  slave_list->tables[s]->count)) {
        if (fVerbose) {
          msr_print("OK", master_list->tables[m]->name,
                    master_list->tables[m]->count, slave_list->tables[s]->name,
                    slave_list->tables[s]->count);
        }
      } else {
        fVerbose = TRUE;
        pre_print(master_dbg, slave_dbg);
        msr_print("NG", master_list->tables[m]->name,
                  master_list->tables[m]->count, slave_list->tables[s]->name,
                  slave_list->tables[s]->count);
        fVerbose = fOVerbose;
      ...
      fVerbose = TRUE;
      pre_print(master_dbg, slave_dbg);
      msr_print("NG", "", "0", slave_list->tables[s]->name,
                slave_list->tables[s]->count);
      fVerbose = fOVerbose;
...
static void lookup_master_slave(char *name, DBG_Struct *dbg, void *dummy) {
...
static NETFILE *connect_dbredirector(DBG_Struct *rdbg) {
  int fh;
  NETFILE *fp = NULL;
