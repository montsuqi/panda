...
static void PutTab(LargeByteString *lbs, int n) {
  for (; n > 0; n--) {
    LBS_EmitString(lbs, "\t");
  }
}

static void PutDim(LargeByteString *lbs) {
...
static void PutItemName(LargeByteString *lbs) {
  int j;
...
static void TableBody(LargeByteString *lbs, ValueStruct *val, size_t arraysize,
                      size_t textsize) {
  int i;
  ValueStruct *tmp;
  Bool fComm;
  ...
  case GL_TYPE_NUMBER:
    PutItemName(lbs);
    sprintf(buff, "numeric(%d,%d)", (int)ValueFixedLength(val),
  ...
  case GL_TYPE_RECORD:
    ...
    for (i = 0; i < ValueRecordSize(val); i++) {
      ...
      if ((!IS_VALUE_VIRTUAL(tmp)) && (!IS_VALUE_ALIAS(tmp))) {
        if (fComm) {
          LBS_EmitString(lbs, ",\n");
        }
        fComm = TRUE;
...
static void MakeCreate(DBG_Struct *dbg, RecordStruct *rec) {
  char ***item, **pk;
  KeyStruct *key;
  LargeByteString *lbs;
  ...
  if ((ValueAttribute(rec->value) & GL_ATTR_VIRTUAL) == 0) {
    ...
    if ((rec->type == RECORD_DB) && ((key = RecordDB(rec)->pkey) != NULL)) {
      item = key->item;
      LBS_EmitString(lbs, ",\n\tprimary key(\n");
      while (*item != NULL) {
        pk = *item;
...
static void CreateTables(char *gname, char *tname) {
  DBG_Struct *dbg;
  RecordStruct *rec;
...
static void Insert(char *gname, char *rname, char *pname) {
  ...
  if ((dbg = GetDBG(gname)) != NULL) {
    ...
    if ((p = mmap(NULL, sb.st_size, PROT_READ, MAP_PRIVATE, fileno(stdin),
                  0)) != NULL) {
      left = sb.st_size;
      while (left > 0) {
...
static ARG_TABLE option[] = {
    ...
    {"dbgroup", STRING, TRUE, (void *)&DBG_Name, N_("dbgroup name")},
    {"dbd", STRING, TRUE, (void *)&DBD_Name, N_("DBD name")},

    {"create", BOOLEAN, TRUE, (void *)&fCreate, N_("execute create table")},
    {"insert", BOOLEAN, TRUE, (void *)&fInsert, N_("execute insert")},

    {"textsize", INTEGER, TRUE, (void *)&TextSize, N_("max of text length")},
    {"arraysize", INTEGER, TRUE, (void *)&ArraySize, N_("max of array")},
    {"record", STRING, TRUE, (void *)&RecordDir, N_("record folder")},
    {"inlang", STRING, TRUE, (void *)&InLang, N_("input data language type")},
