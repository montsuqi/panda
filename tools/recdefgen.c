...
void GenRecord(GtkWidget *widget, int level) {
  long type;
  ...
  if (type == GTK_TYPE_NOTEBOOK) {
  ...
  } else if (type == GTK_TYPE_TOGGLE_BUTTON || type == GTK_TYPE_CHECK_BUTTON ||
    ...
    putLevel(level + 1);
    printf("day int;\n");
  } else if (type == GTK_PANDA_TYPE_PDF || type == GTK_PANDA_TYPE_PIXMAP) {
    putLevel(level + 1);
...
void GenRecordEnd(int l) {
  putLevel(l);
  printf("};\n");
}

void GenRecordDefine(GtkWidget *widget, gpointer data) {
  GtkWidget *child;
  int level;
  long type;
  ...
  if (type == GTK_TYPE_HSEPARATOR || type == GTK_TYPE_VSEPARATOR) {
    return;
  }
  ...
  if (GTK_IS_CONTAINER(widget)) {
    ...
    gtk_container_forall(GTK_CONTAINER(widget), GenRecordDefine,
