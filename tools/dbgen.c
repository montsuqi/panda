...
static void TableBody(ValueStruct *val, size_t arraysize, size_t textsize) {
  ...
  case GL_TYPE_NUMBER:
    PutItemName();
    printf("numeric(%d,%d)", (int)ValueFixedLength(val),
...
static void MakeInsert(RecordStruct *rec) {
  if ((ValueAttribute(rec->value) & GL_ATTR_VIRTUAL) == 0) {
...
static ARG_TABLE option[] = {
    {"create", BOOLEAN, TRUE, (void *)&fCreate, "generate 'create table'"},
    {"drop", BOOLEAN, TRUE, (void *)&fDrop, "generate 'drop table'"},

    {"insert", BOOLEAN, TRUE, (void *)&fInsert, "generate insert script"},

    {"textsize", INTEGER, TRUE, (void *)&TextSize, "'text' length(for COBOL)"},
