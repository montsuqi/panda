...
static ARG_TABLE option[] = {
    ...
    {"json", STRING, TRUE, (void *)&JSONFile, "json input file"},
...
static int MonPushEvent(DBG_Struct *dbg, const char *recfile,
                        const char *datafile) {
  gchar *_buf, *buf;
  gsize size;
  ValueStruct *val;
...
static int MonPushEventJSON(DBG_Struct *dbg) {
  ...
  ret = push_event_via_json(dbg, (const char *)json_object_get_string(event),
                            body);
  json_object_put(obj);
  return ret;
}
