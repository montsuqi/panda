...
void ShowPrintDialog(const char *title, LargeByteString *lbs) {
  GtkWidget *dialog;
  GtkWidget *content;
  GtkWidget *pandapdf;
  gchar *_title;
  ...
  if (!gtk_panda_pdf_set(GTK_PANDA_PDF(pandapdf), LBS_Size(lbs),
                         LBS_Body(lbs))) {
  ...
  dialog =
      gtk_dialog_new_with_buttons(_("Preview"), NULL, GTK_DIALOG_MODAL,
...
static void _Print(const char *title, const char *printer, int copies,
                   LargeByteString *lbs) {
  GtkWidget *pandapdf;
  char buf[1024];
...
static gboolean CheckPrinter(const char *name) {
...
void Print(const char *oid, const char *title, const char *pname,
           LargeByteString *lbs) {
