...
static gboolean check_json_object(json_object *config_obj,
                                  enum json_type type) {
  if (config_obj == NULL) {
    return FALSE;
  }
...
static json_object *_new_server() {
  json_object *child;

  child = json_object_new_object();
  json_object_object_add(child, "description",
                         json_object_new_string("default"));
  json_object_object_add(child, "authuri",
                         json_object_new_string("http://localhost:8000/rpc/"));
  json_object_object_add(
      child, "style",
      json_object_new_string("/usr/share/panda-client/jma-receipt.rc"));
  json_object_object_add(child, "gtkrc", json_object_new_string(""));
  json_object_object_add(child, "fontname",
                         json_object_new_string("Takaoゴシック 10"));
  ...
  json_object_object_add(child, "show_startup_message",
...
void gl_config_set_index(int i) {
  json_object_object_add(config_obj, "index", json_object_new_int(i));
}

static json_object *new_server(int i) {
  json_object *child;
  char desc[256];
...
void gl_config_del_server(int i) {
  json_object *list;
...
int gl_config_get_config_nums(void) {
  json_object *list;
...
void gl_config_set_string(int i, const char *key, const char *val) {
  json_object *list, *server;
...
const char *gl_config_get_string(int i, const char *key) {
...
void gl_config_set_int(int i, const char *key, int val) {
  json_object *list, *server;
...
int gl_config_get_int(int i, const char *key) {
  json_object *list, *server, *child;
...
void gl_config_set_boolean(int i, const char *key, gboolean val) {
  json_object *list, *server;
...
gboolean gl_config_get_boolean(int i, const char *key) {
  json_object *list, *server, *child;
...
gboolean gl_config_get_boolean_val(int i, const char *key, gboolean val) {
  json_object *list, *server, *child;
...
extern int GetConfigIndexByDesc(const char *desc) {
  int i, n;
