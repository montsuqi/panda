...
static void _Error(char *msg, char *fn, int line) {
  printf("%s:%d:%s\n", fn, line, msg);
}
#undef Error
#define Error(msg)                                                             \
  {                                                                            \
    fError = TRUE;                                                             \
    _Error((msg), StyleFileName, StylecLine);                                  \
  }

static int HexStringToInt(char *p, size_t len) {
  int ret;
...
static void ParColor(GdkColor *color) {
  size_t len;
  char *p;
  ...
  do {
    ...
    case T_STYLE:
      if (GetSymbol == T_SCONST) {
        if ((st = g_hash_table_lookup(Styles, StyleComSymbol)) == NULL) {
...
void ghfunc_styles_free(char *name, GtkStyle *value, gpointer user_data) {
  xfree(name);
  g_object_unref(value);
  (void)user_data; /* escape warning */
}

void StyleParserTerm(void) {
  if (Styles != NULL) {
...
#ifdef MAIN
static char *state[] = {"NORMAL", "ACTIVE", "PRELIGHT", "SELECTED",
                        "INSENSITIVE"};

static void PrintStyle(GtkStyle *value) {
  int i;

  printf("fontset = [%X]\n", (int)value->font);
  for (i = 0; i < 5; i++) {
    printf("fg[%s] = { %d, %d, %d }\n", state[i], value->fg[i].red,
           value->fg[i].green, value->fg[i].blue);
    printf("bg[%s] = { %d, %d, %d }\n", state[i], value->bg[i].red,
           value->bg[i].green, value->bg[i].blue);
  }
}
static void Dump(char *name, GtkStyle *value, gpointer user_data) {
  printf("name = [%s]\n", name);
  PrintStyle(value);
}
