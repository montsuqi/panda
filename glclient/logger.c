...
void InitLogger_via_FileName(const char *filename) {
  if (fp == NULL) {
...
void SetLogLevel(int _level) { level = _level; }

void SetErrorFunc(void (*func)(const char *, ...)) { ErrorFunc = func; }
...
void logger(int _level, const char *file, int line, const char *format, ...) {
  struct timeval tv;
  struct tm now;
