...
static void StartSession(GLProtocol *ctx) {
  ...
  json_object_object_add(obj, "app_rpc_endpoint_uri",
                         json_object_new_string(GLP_GetRPCURI(ctx)));
  json_object_object_add(obj, "app_rest_api_uri_root",
                         json_object_new_string(GLP_GetRESTURI(ctx)));
  printf("%s", json_object_to_json_string_ext(obj, JSON_C_TO_STRING_PLAIN));
  json_object_put(obj);
}

static void GetWindow(GLProtocol *ctx) {
  json_object *obj;
...
static void ListDownloads(GLProtocol *ctx) {
  json_object *obj;
...
static void GetMessage(GLProtocol *ctx) {
...
static void SendEvent(GLProtocol *ctx, char *file) {
  json_object *obj, *res;
  gchar *buf, *jsonstr;
  gsize size;
...
static void EndSession(GLProtocol *ctx) { RPC_EndSession(ctx); }
...
static GOptionEntry entries[] = {
    {"authuri", 0, 0, G_OPTION_ARG_STRING, &AUTHURI, "authuri", NULL},
    {"rpcuri", 0, 0, G_OPTION_ARG_STRING, &RPCURI, "rpcuri", NULL},
    {"resturi", 0, 0, G_OPTION_ARG_STRING, &RESTURI, "resturi", NULL},
    {"seesionid", 0, 0, G_OPTION_ARG_STRING, &SESSIONID, "sessionid", NULL},
    {"rpcid", 0, 0, G_OPTION_ARG_INT, &RPCID, "rpcid", NULL},
    {"user", 0, 0, G_OPTION_ARG_STRING, &USER, "user", NULL},
    {"pass", 0, 0, G_OPTION_ARG_STRING, &PASS, "pass", NULL},
#ifdef USE_SSL
    {"certfile", 0, 0, G_OPTION_ARG_STRING, &CERTFILE, "certfile", NULL},
    {"keyfile", 0, 0, G_OPTION_ARG_STRING, &KEYFILE, "keyfile", NULL},
    {"keypass", 0, 0, G_OPTION_ARG_STRING, &KEYPASS, "keypass", NULL},
    {"cafile", 0, 0, G_OPTION_ARG_STRING, &CAFILE, "cafile", NULL},
#endif
    {NULL}};
