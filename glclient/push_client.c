...
static void event_handler(json_object *ent) {
  json_object *obj;
...
static void message_handler(char *in) {
...
static void warn_reconnect() {
  ...
  json_object_object_add(data, "event",
                         json_object_new_string("websocket_reconnect"));
...
static int callback_push_receive(struct lws *wsi,
                                 ...
                                 void *lws_user, void *in, size_t len) {
