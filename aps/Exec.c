...
static void GetApplication(MessageHandler *handler, NETFILE *fp,
  ...
  for (i = 0; i < node->cWindow; i++) {
    if ((node->scrrec[i] != NULL) && (node->scrrec[i]->value != NULL)) {
      ConvSetRecName(handler->conv, node->scrrec[i]->name);
...
static void SignalHandler(int dummy) { longjmp(SubError, 1); }
...
static Bool _ExecuteProcess(MessageHandler *handler, ProcessNode *node) {
  ...
  module =
      ValueToString(GetItemLongName(node->mcprec->value, "dc.module"), NULL);
...
static MessageHandlerClass Handler = {
    "Exec",  _ReadyExecute, _ExecuteProcess, _StartBatch, _ReadyDC,
    _StopDC, NULL,          _ReadyDB,        _StopDB,     NULL};

extern MessageHandlerClass *Exec(void) {
  if (LibPath == NULL) {
    ExecPath = getenv("APS_EXEC_PATH");
  } else {
    ExecPath = LibPath;
