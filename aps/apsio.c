...
typedef struct _SessionCache {
  char *term;
  struct _SessionCache *prev, *next;
...
static Bool SendRecord(NETFILE *fp, RecordStruct *rec) {
  Bool ret;

  if (rec != NULL) {
    dbgprintf("Send rec = [%s] size[%d]\n", rec->name,
              NativeSizeValue(NULL, rec->value));
...
static Bool RecvRecord(NETFILE *fp, RecordStruct *rec) {
  Bool ret;
...
static Bool GetWFCTerm(NETFILE *fp, ProcessNode *node) {
  MessageHeader hdr;
  int i;
  PacketClass c;
  ValueStruct *e;
  ...
  while (1) {
    ...
    case APS_WINDOW_STACK:
      node->w.sp = RecvInt(fp);
      ON_IO_ERROR(fp, badio);
      for (i = 0; i < node->w.sp; i++) {
        node->w.s[i].puttype = RecvChar(fp);
        ON_IO_ERROR(fp, badio);
    ...
    case APS_LINKDATA:
      dbgmsg("LINKDATA");
      RecvRecord(fp, node->linkrec);
      ON_IO_ERROR(fp, badio);
      break;
    case APS_SPADATA:
      dbgmsg("SPADATA");
      RecvRecord(fp, node->sparec);
      ON_IO_ERROR(fp, badio);
      break;
    case APS_SCRDATA:
      dbgmsg("SCRDATA");
      for (i = 0; i < node->cWindow; i++) {
        RecvRecord(fp, node->scrrec[i]);
        ON_IO_ERROR(fp, badio);
      }
      break;
    case APS_END:
...
extern Bool GetWFC(NETFILE *fp, ProcessNode *node) {
  PacketClass c;
  Bool ret;
...
static void PutWFCTerm(NETFILE *fp, ProcessNode *node) {
  int i;
  PacketClass c;
  ValueStruct *e;
  ...
  while (1) {
    ...
    case APS_LINKDATA:
      dbgmsg("LINKDATA");
      SendRecord(fp, node->linkrec);
      ON_IO_ERROR(fp, badio);
      break;
    case APS_SPADATA:
      dbgmsg("SPADATA");
      SendRecord(fp, node->sparec);
      ON_IO_ERROR(fp, badio);
      break;
    case APS_SCRDATA:
      dbgmsg("SCRDATA");
      for (i = 0; i < ThisLD->cWindow; i++) {
        SendRecord(fp, node->scrrec[i]);
        ON_IO_ERROR(fp, badio);
      }
      break;
    case APS_END:
