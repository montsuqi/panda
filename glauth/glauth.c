...
static void Session(NETFILE *fp) {
  size_t size;
  ...
  if (Recv(fp, &size, sizeof(size)) > 0) {
    Recv(fp, user, size);
    user[size] = 0;
    RecvnString(fp, sizeof(pass), pass);
...
static Bool CheckPassword(void) {
  ...
  if (stat(PasswordFile, &stbuf) == 0) {
    if ((stmtime != stbuf.st_mtime) || (stctime != stbuf.st_ctime) ||
...
static void _InitPasswd(int dummy) {
...
static ARG_TABLE option[] = {
    {"port", STRING, TRUE, (void *)&PortNumber, "ポート番号"},
    {"back", INTEGER, TRUE, (void *)&Back, "接続待ちキューの数"},
    {"password", STRING, TRUE, (void *)&PasswordFile, "パスワードファイル"},
