...
static void CheckAPS_Node(LD_Node *ld, int ix) {
  APS_Node *aps;
  SessionData *data;
...
static void ClearAPS_Node(LD_Node *ld, int ix) {
  APS_Node *aps;
...
static Bool CheckAPS(PacketClass klass, LD_Node *ld, int ix) {
  ...
  aps = &ld->aps[ix];
  if (aps->fp != NULL) {
    SendPacketClass(aps->fp, klass);
    ON_IO_ERROR(aps->fp, badio);
    return TRUE;
  }
badio:
  aps->fp = NULL;
...
static Bool PutAPS(LD_Node *ld, int ix, SessionData *data) {
  ...
  dbgmsg("EVENTDATA");
  SendPacketClass(fp, APS_EVENTDATA);
  ON_IO_ERROR(fp, badio);
  SendChar(fp, data->hdr->command);
  ON_IO_ERROR(fp, badio);
  SendString(fp, data->hdr->uuid);
  ON_IO_ERROR(fp, badio);
  SendString(fp, data->hdr->tenant);
  ON_IO_ERROR(fp, badio);
  SendString(fp, data->hdr->host);
  ON_IO_ERROR(fp, badio);
  SendString(fp, data->hdr->tempdir);
  ON_IO_ERROR(fp, badio);
  SendString(fp, data->hdr->user);
  ON_IO_ERROR(fp, badio);
  SendString(fp, data->hdr->window);
  ON_IO_ERROR(fp, badio);
  SendString(fp, data->hdr->widget);
  ON_IO_ERROR(fp, badio);
  SendString(fp, data->hdr->event);
  ON_IO_ERROR(fp, badio);
  SendChar(fp, data->hdr->dbstatus);
  ON_IO_ERROR(fp, badio);

  dbgmsg("WINDOW_STACK");
  SendPacketClass(fp, APS_WINDOW_STACK);
  ON_IO_ERROR(fp, badio);
  SendInt(fp, data->w.sp);
  ON_IO_ERROR(fp, badio);
  for (i = 0; i < data->w.sp; i++) {
    SendChar(fp, data->w.s[i].puttype);
    ON_IO_ERROR(fp, badio);
    SendString(fp, data->w.s[i].window);
    ON_IO_ERROR(fp, badio);
  }

  dbgmsg("SCRDATA");
  SendPacketClass(fp, APS_SCRDATA);
  ON_IO_ERROR(fp, badio);
  for (i = 0; i < data->ld->info->cWindow; i++) {
    if (data->scrdata[i] != NULL) {
      dbgprintf("[%s]", data->ld->info->windows[i]->name);
      SendLBS(fp, data->scrdata[i]);
      ON_IO_ERROR(fp, badio);
    }
  }
  ...
  if (data->linkdata != NULL) {
    SendPacketClass(fp, APS_LINKDATA);
    ON_IO_ERROR(fp, badio);
    SendLBS(fp, data->linkdata);
    ON_IO_ERROR(fp, badio);
  }
  ...
  if (data->spa == NULL) {
    ...
    LBS_ReserveSize(data->spa, NativeSizeValue(NULL, ld->info->sparec->value),
                    FALSE);
    NativePackValue(NULL, LBS_Body(data->spa), ld->info->sparec->value);
  }
  SendPacketClass(fp, APS_SPADATA);
  ON_IO_ERROR(fp, badio);
  SendLBS(fp, data->spa);
  ON_IO_ERROR(fp, badio);
...
static Bool GetAPS_Control(LD_Node *ld, int ix, SessionData *data) {
  APS_Node *aps;
  PacketClass c;
  NETFILE *fp;
  ...
  ON_IO_ERROR(fp, badio);
  if (c != APS_CTRLDATA) {
    Message("aps die [%s@%s] %s:%s:%s", data->hdr->user, data->hdr->uuid,
  ...
  ON_IO_ERROR(fp, badio);
  data->hdr->dbstatus = (char)RecvChar(fp);
  ON_IO_ERROR(fp, badio);
  data->hdr->puttype = RecvChar(fp);
  ON_IO_ERROR(fp, badio);
...
static Bool GetAPS_Value(NETFILE *fpLD, SessionData *data) {
  ...
  dbgmsg("LINKDATA");
  SendPacketClass(fpLD, APS_LINKDATA);
  ON_IO_ERROR(fpLD, badio);
  RecvLBS(fpLD, data->linkdata);
  ON_IO_ERROR(fpLD, badio);

  dbgmsg("SPADATA");
  SendPacketClass(fpLD, APS_SPADATA);
  ON_IO_ERROR(fpLD, badio);
  RecvLBS(fpLD, data->spa);
  ON_IO_ERROR(fpLD, badio);

  dbgmsg("SCRDATA");
  SendPacketClass(fpLD, APS_SCRDATA);
  ON_IO_ERROR(fpLD, badio);
  for (i = 0; i < data->cWindow; i++) {
    if (data->scrdata[i] != NULL) {
      RecvLBS(fpLD, data->scrdata[i]);
      ON_IO_ERROR(fpLD, badio);
    }
  }
  dbgmsg("END");
...
static SessionData *SelectData(Queue *que, int ix) {
  SessionData *data;
#ifdef APS_STICK
  LD_Node *ld;
#endif
  ...
  while ((data = GetElement(que)) != NULL) {
    ...
    if (data->type != SESSION_TYPE_CHECK) {
      if (((ThisEnv->mlevel == MULTI_NO) || (ThisEnv->mlevel == MULTI_ID))) {
        break;
      }
    }
  }
...
static void SendTermMessage(SessionData *data, MQ_Node *mq, int ix) {
  LD_Node *ld, *newld;
  NETFILE *fp;
  ...
  if (fp == NULL) {
    ...
    data->apsid = -1;
    data->retry++;
    if ((MaxTransactionRetry > 0) && (MaxTransactionRetry < data->retry)) {
...
static void SendAPIMessage(SessionData *data, MQ_Node *mq, int ix) {
  LD_Node *ld;
  NETFILE *fp;
  PacketClass c;
  ...
  if (!CheckAPS(APS_API, ld, ix)) {
    ...
    if (c == APS_NULL) {
      ...
      SendString(fp, data->hdr->user);
      ON_IO_ERROR(fp, badio);
      SendLBS(fp, data->apidata->rec);
      ON_IO_ERROR(fp, badio);
      RecvLBS(fp, data->apidata->rec);
      ON_IO_ERROR(fp, badio);
...
static void MessageThread(APS_Start *aps) {
  SessionData *data;
  int ix;
  MQ_Node *mq;
...
static int TableComp(char **x, char **y) {
