...
static TokenTable DB_Tokentable[] = {{"primary", T_PRIMARY},
                                     {"path", T_PATH},
                                     {"use", T_USE},
...
static char ***ParKeyItem(CURFILE *in, ValueStruct *root) {
  char **name, **p;
  size_t count, rcount;
  char ***ret, ***r;
  char *elm;
...
static void ParKey(CURFILE *in, RecordStruct *rec) {
  DB_Struct *db;
  KeyStruct *skey;
...
static DB_Struct *InitDB_Struct(char *gname) {
  ...
  if (gname == NULL) {
    ret->gname = NULL;
  } else if ((ret->dbg = GetDBG(gname)) == NULL) {
...
static void InsertBuildIn(PathStruct *ret, char *name, int func) {
  DB_Operation *op;
...
static Bool fTop;
static int SCRIPT_Lex(CURFILE *in) {
  int c;
...
static LargeByteString *_ParSCRIPT(CURFILE *in, RecordStruct *rec,
                                   ValueStruct *argp, ValueStruct *argf) {
  LargeByteString *scr;
  int pc, c;
...
static LargeByteString *ParScript(CURFILE *in, RecordStruct *rec,
                                  ValueStruct *argp, ValueStruct *argf) {
  LargeByteString *ret;
...
static void ParTableOperation(CURFILE *in, RecordStruct *rec,
                              PathStruct *path) {
  int ix;
  DB_Operation **ops, *op;
  ValueStruct *value;
...
static void ParPath(CURFILE *in, RecordStruct *rec, int usage) {
  ...
  g_hash_table_insert(RecordDB(rec)->paths, path->name,
                      (void *)((long)pcount + 1));
  RecordDB(rec)->pcount++;
...
quit:
  if ((ret->type == RECORD_DB) && (RecordDB(ret)->pcount == 0)) {
...
static void ResolveAlias(RecordStruct *root, ValueStruct *val) {
  char *name;
  int i;
  char *p;
  RecordStruct *use;
