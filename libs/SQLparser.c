...
static ValueStruct *TraceAlias(RecordStruct *rec, ValueStruct *val) {
  char *name;
  char *p;
  RecordStruct *use;
    ...
    if (ValueType(val) == GL_TYPE_ALIAS) {
      ...
      if (*name != '.') {
        if ((p = strchr(name, '.')) != NULL) {
          *p = 0;
          if ((use = g_hash_table_lookup(RecordDB(rec)->use, name)) != NULL) {
...
extern LargeByteString *ParSQL(CURFILE *in, RecordStruct *rec,
                               ValueStruct *argp, ValueStruct *argf) {
  LargeByteString *sql;
  ValueStruct *valr, *valp, *valf, *val;
  Bool fInto, fAster, fDeclare;
  size_t mark, current;
  int into;
  int n;
  int pred;
...
extern void SQL_ParserInit(void) { SQL_LexInit(); }
