...
extern void SetValueName(char *name) { strcpy(namebuff, name); }
...
extern void SendValueString(NETFILE *fpComm, ValueStruct *value, char *name,
  ...
  case GL_TYPE_ARRAY:
    for (i = 0; i < ValueArraySize(value); i++) {
      sprintf(name, "[%d]", i);
      SendValueString(fpComm, ValueArrayItem(value, i), name + strlen(name),
                      fName, fType, encode);
    }
    break;
  case GL_TYPE_RECORD:
    for (i = 0; i < ValueRecordSize(value); i++) {
      ...
      SendValueString(fpComm, ValueRecordItem(value, i), name + strlen(name),
                      fName, fType, encode);
    }
    break;
  default:
    if (fName) {
      ...
      if (fType) {
        ...
        case GL_TYPE_DBCODE:
          ...
          break;
        case GL_TYPE_NUMBER:
          sprintf(buff, "number(%d,%d)", (int)ValueFixedLength(value),
