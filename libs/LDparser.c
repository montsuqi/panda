	...
	if		(  GetSymbol  !=  '{'  ) { 
		...
		while	(  GetName  !=  '}'  ) {
			if		(  LD_Token  ==  T_SYMBOL  ) {
				if		(  ( win = SetWindowName(LD_ComSymbol) )  !=  NULL  ) {
					if		(  ( ix = (int)g_hash_table_lookup(ld->whash,win->name) )  ==  0  ) {
						bind = AddWindow(ld,win->name);
						bind->handler = NULL;
						bind->module = NULL;
					} else {
						bind = ld->window[ix-1];
					}
					bind->value = win->Value;
					if		(  g_hash_table_lookup(LD_Table,win->name)  ==  NULL  ) {
						g_hash_table_insert(LD_Table,win->name,ld);
					} else {
			...
			do {
				...
				if		(  (  db = DD_ParserDataDefines(name) )  !=  NULL  ) {
					if		(  g_hash_table_lookup(ld->DB_Table,LD_ComSymbol)  ==  NULL  ) {
						...
						ld->db[ld->cDB] = db;
						ld->cDB ++;
						g_hash_table_insert(ld->DB_Table,StrDup(LD_ComSymbol),db);
					} else {
		...
		switch	(LD_Token) {
		  case	T_NAME:
			if		(  GetName  !=  T_SYMBOL  ) {
				...
				ret->home = NULL;
				ret->wfc = ParPort("localhost",PORT_WFC_APS);
			}
			break;
		  case	T_ARRAYSIZE:
