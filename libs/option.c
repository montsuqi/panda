...
extern void ChangeExt(char *dist, char *org, char *ext) {
  char *p;
...
static char *CheckParam(char *line, char *token) {
  char *ret, *p;
  char buf[MAX_LINE + 1];
...
static void SetVar(ARG_TABLE *option, char *arg) {
  char *endptr;
    ...
    if (*arg == '+') {
      *(Bool *)option->var = FALSE; /* TRUE;*/
      ...
      if (*arg == '-') {
        ...
        *(Bool *)option->var = (*(Bool *)option->var == TRUE) ? FALSE : TRUE;
      }
    }
    break;
...
static void PrintVar(ARG_TABLE option) {
  switch (option.type) {
  case BOOLEAN:
    if (*(Bool *)option.var) {
      printf("ON");
...
static Bool AnalizeLine(ARG_TABLE *tbl, char *line) {
  Bool rc;
  char *arg;
...
extern FILE_LIST *GetOption(ARG_TABLE *tbl, int argc, char **argv, char *help) {
  ...
  while ((argc > 1) || (fpParam != NULL)) {
    if (fpParam != NULL) {
      while (((c = fgetc(fpParam)) != EOF) && (isspace(c)))
        ;
      if (c != EOF) {
        p = q = buff;
        do {
    ...
    if (p == NULL) {
      ...
      if (*p == RESPONSE_FILE_SWITCH) {
        LastArg = NULL;
        fpParam = fopen(p + 1, "r");
      } else if (*p == COMMAND_SWITCH) {
        LastArg = NULL;
        while (*p == COMMAND_SWITCH)
          p++;
        if ((!strcmp(p, "?")) || (!strcmp(p, "h")) || (!strcmp(p, "H"))) {
...
static ARG_TABLE option[] = {
    "a",        STRING, TRUE, (void *)&a, "aaa", "b",        INTEGER, TRUE,
    (void *)&b, "bbb",  "c",  BOOLEAN,    TRUE,  (void *)&c, "ccc",   NULL};
