...
static int (*AskPassFunction_)(char *buf, size_t buflen,
                               const char *prompt) = NULL;
#ifdef USE_PKCS11
static ENGINE *InitEnginePKCS11(const char *pkcs11, const char *pin);
static Bool LoadEnginePKCS11(SSL_CTX *ctx, ENGINE **e, const char *pkcs11,
                             const char *slotstr);
#endif //	USE_PKCS11
#endif //	USE_SSL

#ifdef SSL_1_0_0
int ASN1_STRING_length(const ASN1_STRING *x) { return x->length; }
 ...
 *	socket
 */
static ssize_t FD_Read(NETFILE *fp, void *buff, size_t size) {
  ssize_t ret;
...
static ssize_t FD_Write(NETFILE *fp, void *buff, size_t size) {
  ssize_t ret;
...
static void FD_Close(NETFILE *fp) { close(fp->fd); }
...
extern NETFILE *SocketToNet(int fd) {
  NETFILE *fp;
...
extern NETFILE *FileToNet(int fd) {
  NETFILE *fp;
...
static void SSL_Error(char *format, ...) {
...
extern char *GetSSLErrorMessage(void) { return ssl_error_message; }

static void SSL_Warning(char *format, ...) {
  char buff[SSL_MESSAGE_SIZE];
  va_list va;
...
extern char *GetSSLWarningMessage(void) { return ssl_warning_message; }

#define ASKPASS_PROMPT _("Please input key passphrase:")
#define ASKPASS_PROMPT_RETRY _("Verify ERROR!\nPlease input key passphrase:")

extern void SetAskPassFunction(int (*func)(char *buf, size_t sz,
                                           const char *prompt)) {
  AskPassFunction_ = func;
}

static int AskPassFunction(char *buf, size_t sz, const char *prompt) {
...
static char *GetPasswordString(char *buf, size_t sz, const char *prompt) {
...
static int passphrase_callback(char *buf, int buflen, int flag,
                               void *userdata) {
  return AskPassFunction(buf, (size_t)buflen, (const char *)userdata);
}

static ssize_t SSL_Read(NETFILE *fp, void *buff, size_t size) {
  ssize_t ret;
  unsigned long err;
...
static ssize_t SSL_Write(NETFILE *fp, void *buff, size_t size) {
  ssize_t ret;
  unsigned long err;
...
static void SSL_Close(NETFILE *fp) {
  if (fp->peer_cert)
...
static const char *GetSSLErrorString() {
  const char *msg;
  int e;
...
static char *GetSubjectFromCertificate_X509_NAME_online(X509 *cert) {
  X509_NAME *subject;
  char *ret = NULL;
...
static int GetCommonNameFromCertificate__(X509 *cert, char *name, size_t len) {
  X509_NAME *subject;
  ...
  for (i = 0; i < count; i++) {
    if ((entry = X509_NAME_get_entry(subject, i)) == NULL)
      break;
...
static int CheckSubjectAltName(X509_EXTENSION *ext, const char *hostname) {
  ...
  values = meth->i2v(meth, meth->d2i(NULL, (const unsigned char **)&data, len),
                     NULL);
  if (values == NULL)
    return FALSE;
  for (i = 0; i < sk_CONF_VALUE_num(values); i++) {
    ...
    if (strcmp(value->name, "DNS") != 0)
      continue;
    if (strcasecmp(value->value, hostname) != 0)
      continue;
    ok = TRUE;
    break;
  }
...
Bool CheckHostnameInCertificate(X509 *cert, const char *hostname) {
  ...
  if (strcasecmp(hostname, "localhost") == 0)
    return TRUE;
  if (strcasecmp(hostname, "127.0.0.1") == 0)
    return TRUE;
  if (strcasecmp(hostname, "::1") == 0)
    return TRUE;
...
Bool StartSSLClientSession(NETFILE *fp, const char *hostname) {
  X509 *cert;
  Bool id_ok = FALSE;
...
static void CheckValidPeriod(X509 *cert) {
  char buf[256];
  long valid;
  valid =
      (long)difftime(asn1_time_to_time(X509_get_notAfter(cert)), time(NULL));
...
static int _VerifyCallBack(int ok, X509_STORE_CTX *ctx) {
  char buf[256];
  X509 *err_cert;
  BIO *bio_err;
  char *ptr = NULL;
  ...
  case X509_V_ERR_UNABLE_TO_GET_ISSUER_CERT_LOCALLY:
    X509_NAME_oneline(
        X509_get_issuer_name(X509_STORE_CTX_get_current_cert(ctx)), buf,
        sizeof buf);
  ...
  case X509_V_ERR_ERROR_IN_CERT_NOT_BEFORE_FIELD:
    ...
    ASN1_TIME_print(bio_err,
                    X509_get_notBefore(X509_STORE_CTX_get_current_cert(ctx)));
  ...
  case X509_V_ERR_ERROR_IN_CERT_NOT_AFTER_FIELD:
    ...
    ASN1_TIME_print(bio_err,
                    X509_get_notAfter(X509_STORE_CTX_get_current_cert(ctx)));
    length = BIO_get_mem_data(bio_err, &ptr);
    memcpy(printable, ptr, length - 1);
    SSL_Error("%s\n", printable);
    break;
...
static int LocalVerifyCallBack(int ok, X509_STORE_CTX *ctx) {
  char buf[256];
  X509 *err_cert;
  int err, depth;
...
static int RemoteVerifyCallBack(int ok, X509_STORE_CTX *ctx) {
  char buf[256];
  X509 *err_cert;
  int err, depth;
...
static int SSL_CTX_use_certificate_with_check(SSL_CTX *ctx, X509 *x509) {
  int ret;
  X509_STORE_CTX *sctx;
...
static int SSL_CTX_use_certificate_file_with_check(SSL_CTX *ctx, char *file,
                                                   int type) {
  FILE *fp;
  X509 *x509;
  X509_STORE_CTX *sctx;
...
static Bool IsPKCS12(const char *file) {
  Bool ret = TRUE;
  EVP_PKEY *key = NULL;
  X509 *cert = NULL;
  BIO *input;
  ...
  if ((input = BIO_new_file(file, "r")) == NULL) {
    if (d2i_PKCS12_bio(input, &p12) == NULL)
      return FALSE;
  }
...
static Bool LoadPKCS12(SSL_CTX *ctx, const char *file) {
  char passbuf[256];
  char *pass = NULL;
  PKCS12 *p12;
  EVP_PKEY *key = NULL;
...
extern SSL_CTX *MakeSSL_CTX(char *key, char *cert, char *cafile, char *capath,
                            char *ciphers) {
  SSL_CTX *ctx = NULL;
  ...
  if (cert != NULL) {
    if (IsPKCS12(cert)) {
      ...
      if (SSL_CTX_use_certificate_file_with_check(ctx, cert,
                                                  SSL_FILETYPE_PEM) <= 0) {
...
static Bool LoadEnginePKCS11(SSL_CTX *ctx, ENGINE **e, const char *p11lib,
                             const char *slotstr) {
...
extern SSL_CTX *MakeSSL_CTX_PKCS11(ENGINE **e, char *pkcs11, char *slotstr,
                                   char *cafile, char *capath, char *ciphers) {
  SSL_CTX *ctx;
  int mode = SSL_VERIFY_NONE;
  ...
  if (LoadEnginePKCS11(ctx, e, pkcs11, slotstr))
    return ctx;
  return NULL;
}
#endif /* USE_PKCS11 */
