...
{
	uuid_t	u;
	char *id,*sql,*e_user,*e_event,*e_pushed_at,*e_data;
	int rc;
	...
	sql = g_strdup_printf("INSERT INTO %s (id,event,user_,pushed_at,data) VALUES ('%s','%s','%s','%s','%s');",MONPUSHEVENT,id,e_user,e_event,e_pushed_at,e_data);
	rc = ExecDBOP(dbg, sql, FALSE, DB_UPDATE);
