...
static Bool CreateLogTable(DBLogCtx *log) {
  ...
  if (!exists) {
    ...
    snprintf(sql, SIZE_BUFF, "CREATE SEQUENCE \"%s_seq\" START 1;",
             log->tablename);
...
static Bool AppendRecord(DBLogCtx *log, DBLogNo no, char *data,
                         char *checkdata) {
  Bool connected = FALSE;
  PGconn *conn = db_connect(log->dbg, &connected);
  ...
  if (no == DBLOG_DEFAULT_NO) {
    snprintf(sql, sql_len,
             "INSERT INTO \"%s\" (no, data, checkdata) "
             "VALUES(nextval('%s_seq'), '%s','%s');",
             log->tablename, log->tablename, esc_data, esc_checkdata);
  } else {
    snprintf(sql, sql_len,
             "INSERT INTO \"%s\" (no, data, checkdata) VALUES(%" PRIu64
             ",'%s','%s');",
...
extern Bool PutNo_DBLog(DBLogCtx *log, DBLogNo no, char *data,
                        char *checkdata) {
  Bool ret;
...
extern Bool GetLatestNo_DBLog(DBLogCtx *log, DBLogNo *no) {
  ...
  *no = DBLOG_DEFAULT_NO;
  ...
  snprintf(sql, SIZE_BUFF, "SELECT max(no) FROM \"%s\";\n", log->tablename);
