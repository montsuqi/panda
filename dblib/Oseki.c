...
ENTER_FUNC;
	...
	if		(  rec->type  !=  RECORD_DB  ) {
		...
		ret = ExecOseki(dbg,ctrl,rec,src,args,ctrl->usage);
	}
LEAVE_FUNC;
	return	(ret);
}
...
ENTER_FUNC;
	...
	if		(  rec->type  !=  RECORD_DB  ) {
		...
		if		(  src  !=  NULL  ) {
			...
			res = _SendCommand(dbg,sql,ctrl->redirect,ctrl->usage);
			if		(  res  <  0  ) {
				dbgmsg("NG");
				ctrl->rc = MCP_BAD_OTHER;
			} else {
				ses = OS_CONN(dbg,ctrl->usage);
				if		(  OsekiIsData(ses)  ) {
					dbgmsg("OK");
...
ENTER_FUNC;
	...
	if		(  rec->type  !=  RECORD_DB  ) {
		...
		if		(  src  !=  NULL  ) {
			ctrl->rc = MCP_OK;
			rer = ExecOseki(dbg,ctrl,rec,src,args,ctrl->usage);
		} else {
            sql = NewLBS();
            ...
            LBS_EmitEnd(sql);
			res = _SendCommand(dbg,LBS_Body(sql),ctrl->redirect,ctrl->usage);
			if		(  res  <  0  ) {
				dbgmsg("NG");
				ctrl->rc = MCP_BAD_OTHER;
			} else {
...
ENTER_FUNC;
	...
	if		(  rec->type  !=  RECORD_DB  ) {
		...
		db = rec->opt.db;
		path = db->path[ctrl->pno];
		if		(  ( ix = (int)g_hash_table_lookup(path->opHash,ctrl->func) )  ==  0  ) {
