...
ENTER_FUNC;
	...
	if(RequestReadBLOB(NBCONN(dbg),ValueObjectId(obj),&buf,&size) > 0) {
		if (size > 0) {
			...
			xfree(buf);
			JSON_UnPackValue(NULL,jsonstr,v);
			g_free(jsonstr);
			ctrl->rc = MCP_OK;
		}
	}
...
ENTER_FUNC;
	...
	size = JSON_SizeValue(NULL,v);
	buf = malloc(size);
	JSON_PackValue(NULL,buf,v);
...
static	DB_OPS	Operations[] = {
	...
	{	"DBSTART",		(DB_FUNC)JSON_BEGIN },
	{	"DBCOMMIT",		(DB_FUNC)JSON_END },
	/*	table operations	*/
	{	"JSONREAD",		_ReadJSON		},
	{	"JSONWRITE",	_WriteJSON		},
...
InitJSONIO(void)
{
	return	(EnterDB_Function("JSONIO",Operations,DB_PARSER_NULL,&Core,"",""));
}
