...
static char *ignore_count_tables[] = {"monbatch",     "monbatch_clog",
                                      "monbatch_log", "monblob",
                                      "monpushevent", NULL};
...
extern PGconn *template1_connect(DBG_Struct *dbg) {
...
char **make_pgopts(const char *command, DBG_Struct *dbg) {
  int pgoptc;
  char **pgoptv;
...
static Bool sync_wait(pid_t pg_dump_pid, pid_t restore_pid) {
  pid_t pid;
  int status;
  Bool ret = FALSE;
...
static void err_check(int err_fd) {
  size_t len;
  char *ignore;
  char buff[SIZE_BUFF];
  int i;
...
static void set_envauth(AuthInfo *authinfo) {
...
static void db_dump(int fd, AuthInfo *authinfo, char **argv) {
  char *sql;
...
static void db_restore(int fd, AuthInfo *authinfo, char **argv) {
...
extern int dbactivity(DBG_Struct *dbg) {
  PGconn *conn;
  PGresult *res;
  int ret = 0;
  char sql[SIZE_BUFF];
...
static AuthInfo *NewAuthInfo(void) {
  AuthInfo *authinfo;
...
extern Bool db_sync(char **master_argv, AuthInfo *master_authinfo,
                    char **slave_argv, AuthInfo *slave_authinfo, int check) {
  struct sigaction sa;
...
static char *tableType(table_TYPE types) {
  char *type;
...
TableList *NewTableList(int count) {
...
char *queryTableList(table_TYPE types) {
  char *sql;
  char *type;
...
static Bool IgnoreCountTables(char *TableName) {
  int i;
  char *ignore;
...
static void TableCount(PGconn *conn, TableList *table_list) {
  int i;
...
TableList *getTableList(PGconn *conn) {
  PGresult *res;
  TableList *table_list;
  Table *table;
  char *sql;
