...
extern DBMasterThread *NewDBMasterThread(DBG_INSTANCE *dbg, char *auth_url,
                                         Port *port, int backlog) {
...
static Bool SendLogFunc(void *userdata, DBLogRecord *rec) {
...
static Bool Authentication(DBMasterThread *ctx, NETFILE *client, int *ver) {
  PacketClass cmd;
  Bool ret = FALSE;
  ...
  if (cmd != DBREPLICATION_COMMAND_AUTH) {
    goto _io_error;
  }
...
static Bool RecvRequest(int ver, DBMasterThread *ctx, NETFILE *client,
                        DBLogNo *no) {
  PacketClass cmd;
  Bool ret = FALSE;
  ENTER_FUNC;
...
static void *LogSendWorkerMain(void *userdata) {
...
static void StartLogSendWorker(DBMasterThread *ctx, int client_fd) {
  pthread_t worker;
...
static void *DBMasterThreadMain(void *userdata) {
  DBMasterThread *ctx = (DBMasterThread *)userdata;
